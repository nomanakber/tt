...
class Cnnumber extends \Magento\Framework\App\Action\Action{
                ...
                if($error == ""){
                    if($response['result']['status'] == "1"){
                        try{
    						...
    						$btn = '<button class="button btn-bevoid" id="TcsCouriershippingBtn_'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" logistic-type="'.$logistic_type.'" data-id="'.$order_id.'" data-cnnumber="'.$cn_id.'"><span>VOID</span></button>';
    						$text_cn = ucfirst($logistic_type).@$cn_status.'<div style="margin-top:3px;">
    							<a href="'.$print.'">Print&nbsp;CN</a>&nbsp;&nbsp;&nbsp;<a href="'.$tracking.'">Tracking</a>
    						</div>';
    						
    						$cn_text = $btn.'<div style="font-weight:bold; margin-top:5px;" id="TcsCouriershippingCN_'.$order_id.'">'.$text_cn.'</div>';
        					$btn = '<span class="button btn-bevoid" id="TcsCouriershippingBtn_'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" logistic-type="'.$logistic_type.'" data-id="'.$order_id.'" data-cnnumber="'.$cn_id.'"><span style="color: #007bdb;cursor: pointer;">VOID</span></span>';
                			...
                			if(!empty($cn_id)){
                                ...
                                if (!$orderShip->canShip()) {
                            		...
                            		echo json_encode([
                            			'void_url' => $this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid',
                            			'success'=>$success,
                            			'cn_id'=>$cn_id,
                            			'cn_text' => $cn_text,
                            			'oms' => $oms,
	...
	private function getApiCN($order_id,$service_type,$charges,$insurance,$city,$cod_amount,$collect_cash){
		...
		try{
			...
			//$comments="<b>ResponseDescription</b> : <b>Request Processed Successfully</b><br><b>UniqueID</b> : 1b71be6b-029a-4f73-a915-5a3d02f7b8e9<br><b>ApprovalCode</b> : 393244<br><b>CardBrand</b> : Visa<br><b>CardToken</b> : 4075695454667495<br>";

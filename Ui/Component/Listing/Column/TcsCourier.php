...
namespace TcsCourier\Shipping\Ui\Component\Listing\Column;
...
class TcsCourier extends Column
{
    ...
    public function __construct(ContextInterface $context, UiComponentFactory $uiComponentFactory, OrderRepositoryInterface $orderRepository, SearchCriteriaBuilder $criteria,UrlInterface $urlBuilder, array $components = [], array $data = [])
    {
		...
		$this->helperData = $helperData = $objectManager->create('TcsCourier\Shipping\Helper\Data');
	...
	public function getCityList(){
		...
		$cityList = $this->objectManager->create('TcsCourier\Shipping\Helper\Data')->getCityList();
    ...
    public function prepareDataSource(array $dataSource)
    {
		...
		if( $this->isEnabled ){ 
			...
			$order_codes = [];
				...
				//	$btncn = '<button class="button btn-TcsCourier" id="TcsCouriershippingBtn_'.$order_id.'"><span>TcsCourier</span></button>';
				
				    $btncn = '<button id="tracking" class="button btn-TcsCourier" id="btn-submit-TcsCouriershipping" data-id="'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/cnnumber'.'"><span>TcsCourier</span></button>';
				    
				    $btnhtml = '<div style="font-weight:bold; margin-top:5px;font-size: 12px;width: 105px;" id="TcsCouriershippinggencn_'.$order_id.'">'.$btncn.'</div>';
					$btnhtml = '';
					if( @$cn_data['oms'] != 'Y' && !empty($text_cn) ){
						$btn = '<button class="button btn-bevoid" id="TcsCouriershippingBtn_'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" data-id="'.$order_id.'" data-cnnumber="'.$cn_id.'"><span>VOID</span></button>';
					...
					if(!empty($cn_id)){
    					...
    					$btn = '<button class="button btn-bevoid" id="TcsCouriershippingBtn_'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" data-logistic="'.$logistic_type.'" data-id="'.$order_id.'" data-cnnumber="'.$cn_id.'"><span>VOID</span></button>';
    					$text_cn = ucfirst($logistic_type).@$cn_status.'<div style="margin-top:3px;">
    						<a href="'.$print.'">Print&nbsp;CN</a>&nbsp;&nbsp;&nbsp;<a href="'.$tracking.'">Tracking</a>
    					</div>';
    					
    					$btn = '<span class="button btn-bevoid" id="TcsCouriershippingBtn_'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" data-id="'.$order_id.'" data-cnnumber="'.$cn_id.'"><span style="color: #007bdb;cursor: pointer;">VOID</span></span>';

    					$text_cn = $cn_id.'  </br> <button id="tracking" data-cn="'.$cn_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/tracking'.'" data-order="'.$order_id.'">Tracking</button>  <button id="cancel" data-cn="'.$cn_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" data-order="'.$order_id.'">Cancel</button>';

    					$btnhtml = '<div style="font-weight:bold; margin-top:5px;font-size: 12px;width: 105px;" id="TcsCouriershippinggencn"  class="TcsCouriershippinggencn_'.$order_id.'" data-cn="'.$cn_id.'" data-order="'.$order_id.'">'.$text_cn.'</div>';
    					
    					//$btnhtml = '<button class="button btn-bevoid" id="TcsCouriershippingBtn_'.$order_id.'" data-actionurl="'.$this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/bluevoid'.'" data-id="'.$order_id.'" data-cnnumber="'.$cn_id.'"><span>VOID</span></button>';
					}
				...
				//	$item[$fieldName . '_html'] = $btn.'<div style="font-weight:bold; margin-top:5px;" id="TcsCouriershippingCN_'.$order_id.'">'.$text_cn.'</div>'.$statsusdata;
				...
				//	$item[$fieldName . '_totalamount'] = number_format($item['grand_total'],2,'.','');
					...
					$item[$fieldName . '_formaction'] = $this->urlBuilder->getBaseUrl().'TcsCouriershipping/api/cnnumber/id/'.$order_id;	
